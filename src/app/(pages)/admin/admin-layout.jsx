...
import { Bell, Menu, ChevronDown, Settings, HelpCircle, LogOut, User, BarChart2 } from "lucide-react"
...
export default function AdminLayout({ children }) {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    setNotifications([
      {
        id: 1,
        message: "New claim requires urgent review",
        time: "10 minutes ago",
        read: false,
      },
      {
        id: 2,
        message: "Fraud detection system flagged 3 claims",
        time: "1 hour ago",
        read: false,
      },
      {
      ...
      >
        <header className="sticky top-0 z-10 flex justify-between items-center px-6 py-3 bg-[#111111] border-b border-[#2a2a2a]">
          <div className="flex items-center gap-4">
            <Link href="/admin">
              ...
              >
                ...
                <span className="ml-2 text-sm bg-[#6B46C1] px-2 py-0.5 rounded-md">Admin</span>
              </motion.h1>
            </Link>
          </div>
          ...
          <div className="flex items-center gap-4">
            <DropdownMenu>
              ...
              <DropdownMenuContent className="w-80 bg-[#1a1a1a] border border-[#2a2a2a] text-white">
                ...
                <DropdownMenuItem className="justify-center hover:bg-[#2a2a2a]">
                  <Link href="/admin/notifications" className="text-sm text-[#6B46C1]">
            ...
            <Link href="/admin/analytics" target="_blank">
              <motion.div
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <div className="flex items-center gap-2 cursor-pointer hover:bg-[#1a1a1a] p-1.5 rounded-lg transition-colors">
                  ...
                  <div className="hidden md:block">
                    <span className="text-sm font-medium">{isAuthenticated ? user?.name : "Admin"}</span>
                  </div>
              ...
              <DropdownMenuContent className="w-56 bg-[#1a1a1a] border border-[#2a2a2a] text-white">
                <DropdownMenuLabel className="flex flex-col">
                  <span className="font-medium">{isAuthenticated ? user?.name : "Admin"}</span>
                  <span className="text-xs text-[#9ca3af]">Administrator</span>
                </DropdownMenuLabel>
                <DropdownMenuSeparator className="bg-[#2a2a2a]" />
