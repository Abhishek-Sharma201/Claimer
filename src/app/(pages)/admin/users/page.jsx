...
import { Search, ArrowUpDown, Eye, Edit, Trash, Plus, Car, Home, User, Check, X } from "lucide-react"
    ...
    <div className="min-h-screen bg-[#121218] text-white p-4 md:p-6">
      <div className="max-w-7xl mx-auto">
          ...
          <div className="flex gap-2">
            <button
              ...
              className={`px-3 py-2 rounded-md text-sm ${
                filterType === "All" ? "bg-[#6B46C1] text-white" : "bg-[#1E1E24] text-gray-300 hover:bg-[#2A2A33]"
              }`}
            >
              All
            </button>
            <button
              ...
              className={`px-3 py-2 rounded-md text-sm flex items-center ${
                filterType === "Car" ? "bg-[#6B46C1] text-white" : "bg-[#1E1E24] text-gray-300 hover:bg-[#2A2A33]"
              }`}
            >
            ...
            <button
              ...
              className={`px-3 py-2 rounded-md text-sm flex items-center ${
                filterType === "Home" ? "bg-[#6B46C1] text-white" : "bg-[#1E1E24] text-gray-300 hover:bg-[#2A2A33]"
              }`}
            >
            ...
            <table className="w-full">
              <thead>
                <tr className="text-left text-gray-400 border-b border-[#2A2A33]">
                  ...
                  <th className="py-3 px-4 text-sm font-medium">Policy Number</th>
              ...
              <tbody>
                {loading ? (
                  ...
                  filteredUsers.map((user) => (
                    <tr key={user.id} className="border-b border-[#2A2A33] hover:bg-[#23232B]">
        ...
        <div className="flex justify-between items-center mt-4">
          ...
          <div className="flex space-x-2">
            <button className="px-3 py-1 bg-[#1E1E24] hover:bg-[#2A2A33] rounded-md text-sm">Previous</button>
            <button className="px-3 py-1 bg-[#6B46C1] hover:bg-[#805AD5] rounded-md text-sm">1</button>
            <button className="px-3 py-1 bg-[#1E1E24] hover:bg-[#2A2A33] rounded-md text-sm">2</button>
            <button className="px-3 py-1 bg-[#1E1E24] hover:bg-[#2A2A33] rounded-md text-sm">3</button>
            <button className="px-3 py-1 bg-[#1E1E24] hover:bg-[#2A2A33] rounded-md text-sm">Next</button>
          </div>
        </div>
      </div>
      ...
      {showUserModal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-[#1E1E24] rounded-lg w-full max-w-md p-6">
            ...
            <UserForm user={selectedUser} onSave={saveUser} onCancel={closeModal} />
          </div>
        </div>
      )}
      ...
      {showDeleteConfirm && selectedUser && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-[#1E1E24] rounded-lg w-full max-w-md p-6">
            ...
            <p className="mb-6">
              Are you sure you want to delete user <span className="font-bold">{selectedUser.name}</span>? This action
              cannot be undone.
            </p>

            <div className="flex justify-end space-x-3">
              <button onClick={closeModal} className="px-4 py-2 bg-[#2A2A33] hover:bg-[#3A3A43] rounded-md">
                Cancel
              </button>
              <button
...
function UserForm({ user, onSave, onCancel }) {
  const [formData, setFormData] = useState({
    ...
    name: user?.name || "",
    email: user?.email || "",
    policyNumber: user?.policyNumber || "POL-" + Math.floor(10000 + Math.random() * 90000),
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div className="flex justify-end space-x-3 mt-6">
        <button type="button" onClick={onCancel} className="px-4 py-2 bg-[#2A2A33] hover:bg-[#3A3A43] rounded-md">
          Cancel
        </button>
        <button type="submit" className="px-4 py-2 bg-[#6B46C1] hover:bg-[#805AD5] rounded-md flex items-center">
